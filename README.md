...
A **multi-absorbing phase-type (MAPH)** distribution is the joint distribution
of the pair `(τ, κ)`, where `τ` is the absorption time of a continuous-time
Markov chain and `κ ∈ {1,...,n}` is the index of the absorbing state reached.
It reduces to a standard PH distribution when `n = 1`, and is the natural
model for *competing risks*.

`MAPHDist(α, T, D)` stores the initial distribution `α` over transient phases,
the m×m sub-generator `T`, and the m×n absorbing-rate matrix `D`, with the
constraint `T·1_m + D·1_n = 0`.
...
Build an MAPH by choosing an `AbstractPHDist` for each absorbing state and a
marginal probability vector `π`:
...
is realized by a 2-phase hyperexponential (SCV > 1), hypoexponential (SCV < 1),
or exponential (SCV = 1), following the algorithm in the accompanying paper.
